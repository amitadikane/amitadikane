# Hi, I'm Amit Adikane 👋

**Aspiring Data Scientist | MSc AI & Machine Learning (Scaler Neovarsity / Woolf) | B.Tech Mechanical Engineering**

📍 Aurangabad, Maharashtra, India  
🔗 [LinkedIn](https://www.linkedin.com/in/amit-adikane-4060a91b1/) | 📧 amitadikane@gmail.com

---

### 🚀 About Me

- 🎓 Currently pursuing **MSc in Computer Science: Artificial Intelligence & Machine Learning** at Scaler Neovarsity (ECTS-accredited via Woolf University, Malta)
- 🧠 Building strong foundations in **Machine Learning, Statistics, Feature Engineering, Hypothesis Testing & SQL Analytics**
- 📊 Completed **7 end-to-end business case studies** covering E-commerce, Logistics, Consumer Analytics & Education domain
- 👨‍🏫 Former **Physics Faculty (4.5+ years)** for JEE/NEET — strong analytical thinking & communication skills
- 🔧 Background in **Mechanical Engineering** with hands-on project experience
---

### 🛠️ Tech Stack

**Languages:** Python, SQL, C, C++  
**Data Science:** Pandas, NumPy, Scikit-learn, SciPy, Matplotlib, Seaborn  
**Statistics:** Hypothesis Testing (t-test, ANOVA, Chi-Square), Confidence Intervals, CLT, Linear/Ridge/Lasso Regression  
**Tools:** Jupyter, BigQuery, Git/GitHub, Tableau, Power BI  
**Currently Learning:** Deep Learning, NLP, Computer Vision, ML System Design

---

### 📂 Featured Projects

| Project | Domain | Key Skills | Repo |
|---------|--------|------------|------|
| **Delhivery Logistics** | Supply Chain | Feature Engineering, Aggregation, Outlier Treatment | [Link](https://github.com/Amit14594/delhivery-feature-engineering) |
| **Yulu Bike Sharing** | Mobility | Hypothesis Testing (t-test, ANOVA, Chi-Square) | [Link](https://github.com/Amit14594/yulu-hypothesis-testing) |
| **Jamboree Admission** | Education | Linear Regression, Regularization, Assumption Testing | [Link](https://github.com/Amit14594/jamboree-linear-regression) |
| **Walmart Black Friday** | Retail | Confidence Intervals, Central Limit Theorem | [Link](https://github.com/Amit14594/walmart-black-friday-clt) |
| **Aerofit Treadmill** | Consumer Goods | Contingency Tables, Conditional Probability | [Link](https://github.com/Amit14594/aerofit-probability-analysis) |
| **Netflix Content Strategy** | Media | EDA, Visualization, Business Recommendations | [Link](https://github.com/Amit14594/netflix-eda-visualization) |
| **Target Brazil E-Commerce** | Retail / E-commerce | Advanced SQL, Business Insights | [Link](https://github.com/Amit14594/target-brazil-ecommerce-sql) |

---

### 📈 Current Focus

- Completing Machine Learning module at Scaler (14–18 hrs/day intensive study)
- Reading research papers (recently reviewed: *Big Data Analytics in Weather Forecasting*)
- Building and polishing GitHub portfolio for Data Scientist / Data Analyst roles
- Preparing for full-time opportunities in Data Science / Analytics

---

### 📫 Let's Connect

I'm open to **Data Scientist**, **Data Analyst**, **ML Engineer** and related roles.  
Feel free to reach out via LinkedIn or email!

⭐ If you find any of my projects useful, consider giving them a star!


<!---
Amit14594/Amit14594 is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
